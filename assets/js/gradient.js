...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // === ЗАГОТОВКИ ===
    const templates = {
        rainbow: { name: 'Радужный градиент', colors: ['#ff0000', '#ff8800', '#ffff00', '#6fee34', '#00ffff', '#1f90ff', '#9f66e0'] },
        fire: { name: 'Огненный градиент', colors: ['#ffcc00', '#ff0000'] },
        ice: { name: 'Ледяной градиент', colors: ['#6ca4f3', '#5e6ac0'] },
        ice_2: { name: 'Ледяной градиент', colors: ['#0088ff', '#00ccff', '#0088ff'] },
        ice_cream: { name: 'Пломбир', colors: ['#e31679', '#f5da90'] },
        water: { name: 'Водный градиент', colors: ['#b6c79a', '#89e6ca', '#1d81f2'] },
        spruce: { name: 'Ель', colors: ['#67806e', '#425e6e'] },
        carpet: { name: 'Ковер', colors: ['#bc014f', '#7d4987'] },
        creeper: { name: 'Крипер', colors: ['#e5f62c', '#808a71'] },
        legend: { name: 'Легенда', colors: ['#f697ca', '#8bf291'] },
        experience: { name: 'Опыт', colors: ['#f0d921', '#7aca21'] },
        club_cocktail: { name: 'Клубный коктейль', colors: ['#d06ab1', '#604f7a'] },
        red_caviar: { name: 'Красная икра', colors: ['#c23a00', '#ee796c'] },
        red_velvet: { name: 'Красный бархат', colors: ['#f0407f', '#ab0927'] },
        tropical_beach: { name: 'Тропический пляж', colors: ['#5787d3', '#a5bafd', '#75c36f'] },
        minecraft_the_end: { name: 'minecraft:the_end', colors: ['#e6c389', '#8978e3'] },
        pastel_dessert: { name: 'Пастельный десерт', colors: ['#5b61a1', '#b17b90', '#759881'] },
        magic_stick: { name: 'Волшебная палочка', colors: ['#92b9d3', '#fad8ef', '#a1f8ec'] },
        dont_forget_to_water: { name: 'Не забывайте поливать', colors: ['#815c6d', '#b6b664'] },
        your_name_could_be_here: { name: 'Тут могло быть ваше название', colors: ['#ef9317', '#832d76'] },
        olives: { name: 'Оливки', colors: ['#a4a451', '#79b21c'] },
        bjjjjjj: { name: 'Райский мед', colors: ['#ffea00', '#ef7715'] },
        breeze: { name: 'Бриз', colors: ['#db8f89', '#639bbd'] },
        efir_lol: { name: 'Магия эфира 4-х стихий', colors: ['#7da9b5', '#d1a298', '#87bc47'] },
        monochrome: { name: 'Монохромность', colors: ['#595959', '#242424'] },
        clay_pot: { name: 'Глиняный горшочек', colors: ['#c5bcc8', '#a05d61'] },
        matrix: { name: 'Матрица', colors: ['#303030', '#b1d056', '#303030'] },
        chrome: { name: 'Google Chrome', colors: ['#fa2929', '#ffc800', '#4abe19', '#3394f0'] },
        sakura: { name: 'Сакура', colors: ['#fbb6ef', '#b365bf'] },
        berry_bush: { name: 'Ягодный куст', colors: ['#056f8a', '#669f74', '#dd494c'] },
        i_believe_i_can_fly: { name: '5 минут полет отличный', colors: ['#d7d0d2', '#92a9e8'] },
        the_end: { name: 'Конец', colors: ['#4e117d', '#960cf0', '#4e117d'] },
        de_la_france: { name: 'Де ля Франце', colors: ['#197df9', '#dcb2d9', '#fc0e54'] },
        italian_monster: { name: 'Итальянский монстр', colors: ['#95e660', '#f0eddb', '#e85e5e'] },
        cyber: { name: 'Киберпанк', colors: ['#00ffff', '#ff00ff'] },
        sunset: { name: 'Закатный градиент', colors: ['#ff6b35', '#f7931e', '#ffd700', '#ff6b35'] }
    };
    ...
    function loadHistory() {
        try {
            ...
            // Фильтруем записи, у которых нет colors
    ...
    function addToHistory(colorsArray, name) {
        ...
        const entry = {
            ...
            colors: [...colorsArray], // копируем массив
            name: name || 'Кастомный градиент',
            timestamp: Date.now()
        };
    ...
    function renderHistory() {
        ...
        if (recent.length === 0) {
            historyList.innerHTML = '<p class="mut" style="text-align: center; opacity: 0.5;">История пуста. Используйте градиенты, они будут появляться здесь.</p>';
            return;
        }
        ...
        recent.forEach(entry => {
            ...
            item.innerHTML = `
                ...
                <span class="history-item-name">${entry.name || 'Кастомный градиент'}</span>
                <button class="history-use-btn btn" data-colors='${JSON.stringify(entry.colors)}'>Использовать</button>
            `;
        ...
        document.querySelectorAll('.history-use-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                renderColors();
                updatePreview();
                const name = this.closest('.history-item')?.querySelector('.history-item-name')?.textContent || 'Кастомный градиент';
                addToHistory(colorsArray, name);
                if (window.notifications) {
                    window.notifications.info('Градиент применён', name);
                }
            });
        });
    }
    ...
    function openFullHistoryModal() {
        ...
        const history = loadHistory();
        if (history.length === 0) {
            list.innerHTML = '<p style="text-align: center; opacity: 0.5;">История пуста</p>';
        } else {
            list.innerHTML = history.map(entry => {
                ...
                return `
                    <div class="history-item" style="margin-bottom: 0.3rem;">
                        ...
                        <button class="history-use-btn btn" data-colors='${JSON.stringify(entry.colors)}' style="flex-shrink: 0; font-size: 0.7rem; padding: 0.15rem 0.6rem;">Использовать</button>
                    </div>
                `;
        ...
        // обработчики для кнопок "Использовать" внутри модалки
    ...
    document.getElementById('clearFullHistoryBtn')?.addEventListener('click', async function() {
        let confirmed = await window.modal.alert('Удалить всю историю?');
        if (confirmed) {
            saveHistory([]);
            renderHistory();
            const modal = document.getElementById('fullHistoryModal');
            ...
            if (window.notifications) {
                window.notifications.info('История очищена', 'Все записи удалены');
            }
        }
    });
    ...
    if (historyTitle) {
        ...
        historyTitle.title = 'Нажмите для просмотра всей истории';
    ...
    document.getElementById('clearHistory')?.addEventListener('click', async function() {
        let confirmed = await window.modal.alert('Очистить последние 10 записей?');
        if (confirmed) {
            const history = loadHistory();
            ...
            renderHistory();
            if (window.notifications) {
                window.notifications.info('История очищена', 'Последние 10 записей удалены');
            }
        }
    });
    ...
    function toggleFavorite(templateKey, colorsArray, name) {
        ...
        if (window.notifications) {
            window.notifications.info(
                exists ? 'Убрано из избранного' : 'Добавлено в избранное',
    ...
    function renderFavorites() {
        ...
        if (favorites.length === 0) {
            favoriteList.innerHTML = '<p class="mut" style="text-align: center; opacity: 0.5;">Нет избранных градиентов. Нажмите ★ у заготовки или кастомного градиента.</p>';
            return;
        }
        ...
        favorites.forEach(fav => {
            ...
            item.innerHTML = `
                ...
                <button class="favorite-use-btn btn" data-colors='${JSON.stringify(fav.colors)}' data-name="${fav.name}">Использовать</button>
        ...
        document.querySelectorAll('.favorite-use-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                const colorsArray = JSON.parse(this.dataset.colors);
                const name = this.dataset.name || 'Кастомный градиент';
                colors = [...colorsArray];
                renderColors();
                updatePreview();
                addToHistory(colorsArray, name);
        ...
        document.querySelectorAll('.favorite-remove-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                updateFavoriteButtons();
                if (window.notifications) {
                    window.notifications.info('Убрано из избранного');
                }
            });
        });
    }
    ...
    document.getElementById('clearFavorites')?.addEventListener('click', async function() {
        let confirmed = await window.modal.alert('Очистить всё избранное?');
        if (confirmed) {
            saveFavorites([]);
            renderFavorites();
            updateFavoriteButtons();
            if (window.notifications) {
                window.notifications.info('Избранное очищено', 'Все записи удалены');
            }
        }
    });
    ...
    function applyTemplate(templateKey) {
        ...
        renderColors();
        updatePreview();
        addToHistory(template.colors, template.name);
        if (window.notifications) {
            window.notifications.success('Заготовка применена', template.name);
        }
    }
    ...
    function renderColors() {
        ...
        document.querySelectorAll('.color-item input[type="color"]').forEach(input => {
            input.addEventListener('input', function() {
                ...
                updatePreview();
                // обновляем кнопку избранного для кастомного градиента
        ...
        document.querySelectorAll('.color-hex').forEach(span => {
            span.addEventListener('blur', function() {
                ...
                if (/^#[0-9a-fA-F]{6}$/.test(val)) {
                    ...
                    if (window.notifications) window.notifications.warning('Неверный формат', 'Используй HEX: #ff6b35');
                }
            });
        ...
        document.querySelectorAll('.color-remove').forEach(btn => {
            btn.addEventListener('click', function() {
                const index = parseInt(this.dataset.index);
                if (colors.length <= 2) {
                    if (window.notifications) window.notifications.warning('Нельзя удалить', 'Нужно минимум 2 цвета');
                    return;
                }
    ...
    function updateCustomFavoriteButton() {
        // ищем кнопку избранного для кастомного градиента
        let customBtn = document.getElementById('customFavoriteBtn');
        if (!customBtn) {
            // если нет — создаём в секции избранного или рядом с превью
        ...
        customBtn.onclick = function() {
            const name = textInput.value ? `"${textInput.value}" (кастомный)` : 'Кастомный градиент';
    ...
    document.querySelectorAll('.use-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (template && templates[template]) {
                ...
                this.innerHTML = '<span class="ans-icon"><span style="color: var(--green);">✔ Применено</span></span>';
                setTimeout(() => { this.textContent = 'Использовать'; }, 1500);
            }
        });
    });
    ...
    document.addEventListener('click', function(e) {
        ...
        if (btn) {
            ...
            if (key && templates[key]) {
                toggleFavorite(key, templates[key].colors, templates[key].name);
            }
        }
    });
    ...
    document.getElementById('addColor')?.addEventListener('click', function() {
        ...
        updatePreview();
        updateCustomFavoriteButton();
        if (window.notifications) window.notifications.info('Цвет добавлен', `Всего цветов: ${colors.length}`);
    });
    ...
    document.getElementById('removeColor')?.addEventListener('click', function() {
        ...
        updatePreview();
        updateCustomFavoriteButton();
        if (window.notifications) window.notifications.info('Цвет удалён', `Осталось цветов: ${colors.length}`);
    });

    document.getElementById('clearColors')?.addEventListener('click', function() {
        if (colors.length <= 2) {
            if (window.notifications) window.notifications.warning('Нельзя очистить', 'Нужно минимум 2 цвета');
            return;
        }
        ...
        updatePreview();
        updateCustomFavoriteButton();
        if (window.notifications) window.notifications.info('Очищено', 'Оставлено 2 цвета');
    });
    ...
    document.getElementById('randomColors')?.addEventListener('click', function() {
        ...
        let min = parseInt(minInput?.value) || 2;
        let max = parseInt(maxInput?.value) || 6;
        if (min < 2) min = 2;
        ...
        if (window.notifications) window.notifications.info('Случайные цвета', `Сгенерировано ${colors.length} цветов (${min}-${max})`);
    });
                ...
                template.colors.forEach(color => {
                    ...
                    dot.addEventListener('click', function(e) {
                        ...
                        navigator.clipboard.writeText(hex).then(() => {
                            ...
                            if (window.notifications) window.notifications.success('Цвет скопирован', hex);
    ...
    function validateNumberInput(input) {
        ...
        input.addEventListener('input', function() {
            ...
            if (this.value === '') {
                if (this.id === 'minRandomColorsCount') this.value = '2';
                else if (this.id === 'maxRandomColorsCount') this.value = '6';
            }
            let val = parseInt(this.value);
            if (this.id === 'minRandomColorsCount') {
                if (val < 2) this.value = '2';
                if (val > 8) this.value = '8';
            } else if (this.id === 'maxRandomColorsCount') {
